...
const CrearCuenta = () => {
    ...
    //Acceder al state del authState
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({  //aca vamos a tener un esquema de validacion, que va a ser un objeto de yup
            nombre: Yup.string().required("El nombre es obligatorio"),  //le digo que es un string y que es obligatorio
            email: Yup.string().required("El email es obligatorio").email("El email no es valido"),
            password: Yup.string().required("La contraseña no puede ir vacía").min(6, "La contraseña debe tener al menos 6 caracteres")
        }),
                ...
                >
                    <div className="mb-4">
                        ...
                        <input
                            ...
                            id="nombre" //tiene que ser igual al htmlFor para que cuando dé click en el label, se haga focus en el input
                            value={formik.values.nombre}    //establece los valores del input por default
                            onChange={formik.handleChange}  //evita tener que crear una funcion que tome los valores y los envie al state. Formik lo hace solo
                            onBlur={formik.handleBlur}  //para que al salir del input vacio me muestre una alerta
                        />
                    </div>
                    {formik.touched.nombre && formik.errors.nombre ? (  //si salgo del input vacio o si intento hacer submit, me muestra este error
